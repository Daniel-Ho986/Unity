...
public class EnemyController2a : MonoBehaviour, EnemyController
{
    ...
    void Update()
    {
        ...
        if (isAttacking)
        {
            ...
            if (fireSpinAttack != null)
            {
                ...
                {
                    fireSpinAttack.GetComponent<FireSpin2aScript>().ActivateAttack();
                    attackActivated = true;
                    attacksPerformed += 1;
                }
                else if (attackActivated == true)
                {
                    fireSpinAttack.GetComponent<FireSpin2aScript>().StartAttack();
                    isAttacking = false;
                    attackActivated = false;
