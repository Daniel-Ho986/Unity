...
public class DataStructure1SSManager : MonoBehaviour, DataStructureManager
{
    ...
    private int[] question0Answers;    //Question0: To start, which index do we make the temporary minimum/maximum?
    private int[] question1and3Answers;//Question1/Question3: Which index do we start with?
    private int[] question2Answers;    //Question2: Which index do we compare with?
    private int[] question4Answers;    //Question4: Which is the minimum value: the temp value or other value?
    private bool[] question5Answers;   //Question5: Do we continue searching or do we end this iteration?
                                       //           i.e. Did we search the whole array for the min/max?
    private int[] question6Answers;    //Question6: Where do we place the min/max?

    private int question0Position;      //The position within the list of question0Answers that needs to be
    ...
    void Start()
    {
        ...
        iterationStage = 0; //The nested loop iteration number of the sorting algorithm
        iterations = new int[] { 1, 2 }; //An array of all the main loop iterations
        iterationStatuses = new bool[] { false, false }; //Iteration solved states
        isSorted = false;
        isVisible = true;
        ...
        question0Answers = new int[] { 5, 4 };
        question1and3Answers = new int[] { 4, 3, 2, 1, 3, 2, 5 };
        question2Answers = new int[] {     5, 4, 3, 2, 4, 3, 2 };
        question4Answers = new int[] {     4, 3, 2, 1, 3, 2, 2 };
        question5Answers = new bool[] { true, true, true, false, true, true, false };
        question6Answers = new int[] { 5, 4 };
    ...
    */
    ...
    void Update()
    {
        ...
        if (isSorted == false && space3.tag == "MemorySpace" && isBubbled3 == false)
        {
        ...
        if (iterationStatuses[0] == false && space0.tag == "MemorySpace" && isBubbled0 == false)
        {
            if (space0.GetComponent<BoxScript>().GetNumber() == solutionValues[0])
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    ...
                    phaseArray[i] = iterationPhases2D[4, i];
                }
        ...
        if (iterationStatuses[0] == false && space4.tag == "MemorySpace" && isBubbled4 == false)
        {
            if (space4.GetComponent<BoxScript>().GetNumber() == solutionValues[4])
            {
                ...
                for (int i = 0; i < size; i++)
                {
                    ...
                    //        --> Therefore, 4 is the first index
                    phaseArray[i] = iterationPhases2D[4, i];
                }
        ...
        if (iterationStatuses[1] == false && space1.tag == "MemorySpace" && isBubbled1 == false)
        {
        ...
        if (iterationStatuses[1] == false && space2.tag == "MemorySpace" && isBubbled2 == false)
        {
    ...
    public bool GetAnswerForComparison(int answer, int subStep)
    {
        ...
        {
            //Compare player's answer choice with the answer for question0:
            if (iterationStage == 0)
            {
                //If answer is correct, return true--
                if (answer == question0Answers[question0Position])
                {
                    Debug.Log("CORRECT ANSWER!!! - " + answer + " = "
                                    + question0Answers[question0Position]);
                    //Move down the list of question0Answers in preparation for when
                    //  the next question0 is asked and answered
                    if (question0Position < question0Answers.Length)
                    {
                        question0Position++;
                    }
                    else
            ...
            else if (iterationStage == 1 || iterationStage == 3)
            {
            ...
            //Compare player's answer choice with the answer for question6:
            else if (iterationStage == 5)
            {
                //If answer is correct, return true--
                if (answer == question6Answers[question6Position])
                {
                    Debug.Log("CORRECT ANSWER!!! - " + answer + " = "
                                    + question6Answers[question6Position]);
                    //Move down the list of question6Answers in preparation for when
                    //  the next question6 is asked and answered
                    if (question6Position < question6Answers.Length)
                    {
                        question6Position++;
                    }
                    else
                ...
                else
                {
                    ...
                    //Do not increment the question6Position since the player
                    //  did not answer correctly yet

                    return false;
                }
            }
            //Return false if the iteration stage is a number other than 1, 2, 3, or 5:
            else { return false; }
        }
    ...
    public bool GetAnswerForComparison(bool answer, int subStep)
    {
        if (subStep == iterationStage)
        {
             //Compare player's answer choice with the answers for question5:
            if (iterationStage == 4)
            {
                //If answer is correct, return true--
