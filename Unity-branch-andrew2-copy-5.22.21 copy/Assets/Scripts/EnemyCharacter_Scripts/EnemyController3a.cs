...
public class EnemyController3a : MonoBehaviour, EnemyController
{
    ...
    void Update()
    {
        ...
        if (isAttacking)
        {
            ...
            if (fireSpinAttack != null)
            {
                ...
                {
                    fireSpinAttack.GetComponent<FireSpin3aScript>().ActivateAttack();
                    attackActivated = true;
                    attacksPerformed += 1;
                }
                else if (attackActivated == true)
                {
                    fireSpinAttack.GetComponent<FireSpin3aScript>().StartAttack();
                    isAttacking = false;
                    attackActivated = false;
