...
public class EnemyController1SS : MonoBehaviour, EnemyController
{
    ...
    void Update()
    {
        ...
        if (isAttacking)
        {
            ...
            if (fireSpinAttack != null)
            {
                ...
                {
                    fireSpinAttack.GetComponent<FireSpin1SSScript>().ActivateAttack();
                    attackActivated = true;
                    attacksPerformed += 1;
                }
                else if (attackActivated == true)
                {
                    fireSpinAttack.GetComponent<FireSpin1SSScript>().StartAttack();
                    isAttacking = false;
                    attackActivated = false;
