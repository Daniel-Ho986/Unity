...
public class FireSpin2aScript : MonoBehaviour
{
    ...
    void Update()
    {
        if (attackStarted)
        {
            ...
            if (justStarted)
            {
                if (transform.localScale.x < 5.0f && transform.localScale.y < 5.0f)
                {
                    transform.localScale += new Vector3(0.02f, 0.02f, 0.0f);
                }
